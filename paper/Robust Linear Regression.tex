...
\begin{document}
	...
	\begin{table}[!h]
		...
		\begin{tabular}{|lcccc|}
			...
			\midrule
			TMLE \cite{https://doi.org/10.48550/arxiv.2206.04777} & Adaptive & Likelihood & \cmark & $\mathcal{O}(n)$\\
			\midrule 
	...
	\begin{algorithm}[H]
		...
		$\vtheta_{(0)} \gets \mathcal{N}_{d}(0,\sigma)$\\
		\For{$k \in 1,2,\ldots,T$}
		{
			$\mS_{(k)} \gets \textsc{Subquantile}(\vtheta_{(k)},X)$\\
			\For{$B \in \mathcal{B}$}{
				$\displaystyle L_{(b)} \gets \frac{1}{|\mathcal{B}|}\sum_{b \in B}\nabla f(\vx_b;\vtheta_{(k)},y_b)$
				$\displaystyle \vtheta_{(k+1)} \gets \vtheta_{(k)} - \alpha_{(k)} L_{(b)}$
			}
		}
		\KwRet{$ \vtheta_{(T)} $}
	...
	\begin{minipage}{0.48\textwidth}
		\begin{algorithm}[H]
			\KwInput{Training Epochs $T$,Quantile $p$, Base Learner $\mathcal{L}$}
			\KwOutput{Trained Parameters, $\vtheta_{(T)}$}
			$\vtheta_{(0)} \gets \mathcal{L}(\mX,\vy)$\\
			\For{$k \in \left\{1,2,\dots,T\right\}$}
			{
				$\mS_{(k)} \gets \textsc{Subquantile}(\vtheta_{(k)},\mX)$\\
				$\vtheta_{(k+1)} \gets \mathcal{L}(\mS_{(k)}, \vy_S)$\\
			}
			\KwRet{$ \vtheta_{(T)} $}
			\caption{\textsc{Subq-Base}}
			\label{alg:sqo-ridge}
		\end{algorithm}
		\begin{algorithm}[H]
			\KwInput{Parameters $\vtheta$, Data Matrix: $\mX, (n \times d)$, Convex Loss Function $f$}
			\KwOutput{Subquantile Matrix $S$}
			$\hat{\vnu}_i \gets f(\vx_i;\vtheta,y_i)$ s.t. $\hat{\vnu}_{i-1} \leq \hat{\vnu}_i \leq \hat{\vnu}_{i+1}$\\
			$t \gets \hat{\vnu}_{np}$\\
			Let $\vx_1,...,\vx_{np}$ be $np$ points such that $f(\vx_i;\vtheta,y_i) \leq t$\\
	...
	\begin{restatable}{lemma}{gthetaconvexlemma}
		...
		Let $f(\vx;\vtheta, y)$ be a convex function with respect to $\vtheta$. The elements $\vx_1,\vx_2,\cdots,\vx_n$ are ordered such that $f(\vx_1;\vtheta,y_1) \leq f(\vx_2;\vtheta,y_2) \leq \cdots \leq f(\vx_n;\vtheta,y_n)$. It then follows $g(t_{k+1},\vtheta_k)$ is convex with respect to $\vtheta_k$.
	\end{restatable}
	\begin{proof}
		...
		\begin{align*}
			g(t_{(k+1)},\vtheta_{(k)}) &= t_{(k+1)} - \frac{1}{np}\sum_{i=1}^{n}\left(t_{(k+1)} - f(\vx_i;\vtheta_{(k)},y_i)\right)^+ &&\\
			&= t_{(k+1)} - t_{(k+1)} + \frac{1}{np}\sum_{i=1}^{np}f(\vx_i;\vtheta_{(k)},y_i)
			= \frac{1}{np}\sum_{i=1}^{np}f(\vx_i;\vtheta_{(k)},y_i) &&
		\end{align*}
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			...
			TMLE \cite{https://doi.org/10.48550/arxiv.2206.04777} &$1.094_{(0.1065)}$&$1.323_{(0.0758)}$&$1.578_{(0.0799)}$&$1.984_{(0.2020)}$\\
			...
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.994_{(0.1024)}}$&$\mathbf{0.995_{(0.1227)}}$&$\mathbf{1.012_{(0.1029)}}$&$\mathbf{1.029_{(0.1019)}}$\\
			\midrule 
	...
	As we can see in Table \ref{tab:drug-discovery}, we obtain state of the art results throughout all noise regimes. This makes our model the strongest among the tested, due to our strength throughout the whole range of noises.
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			\toprule 
			\textbf{Objectives}&\multicolumn{2}{c}{(\texttt{Logistic Regression})}&\multicolumn{2}{c}{(\texttt{Logistic Regression})}\\                   
			...
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.950_{(0.0500)}}$&$0.707_{(0.1825)}$&$\mathbf{0.802_{(0.1619)}}$ & $\mathbf{0.875_{(0.1355)}}$ \\
			\midrule 
	...
	\begin{equation}
		f(\vx;\vtheta) = \frac{1}{n} \sum_{i=1}^n \left(1 - y_i \left(\vx_i^\top \vtheta\right)\right)_+
	\end{equation}
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			\toprule 
			\textbf{Objectives}&\multicolumn{2}{c}{\texttt{Enron Spam Classification}}&\multicolumn{2}{c}{\texttt{Adult Dataset}}\\                   
			\cmidrule(rl){2-3}\cmidrule(rl){4-5}
			&\subhead{$\eps = 0.2$}& \subhead{$\eps = 0.4$}& \subhead{$\eps = 0.2$} & \subhead{$\eps = 0.4$}\\ 
			\midrule
			ERM  &$0.633_{(0.0246)}$&$0.496_{(0.0608)}$ & $0.785_{(0.0040)}$ & $0.776_{(0.0033)}$\\
			TERM \cite{li2020tilted} &$\infty$&$\infty$ & $\infty$ & $\infty$\\
			SEVER \cite{DiakonikolasKKLSS19} &$0.649_{(0.0276)}$&$0.528_{(0.0305)}$ & $0.696_{(0.0356)}$ & $0.583_{(0.0825)}$\\
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.815_{(0.0204)}}$&$\mathbf{0.653_{(0.0490)}}$&$\mathbf{0.841_{(0.0185)}}$ & $\mathbf{0.811_{(0.0040)}}$ \\
			\midrule 
			Genie ERM &$0.963_{(0.0049)}$&$0.959_{(0.0041)}$&$0.847_{(0.0033)}$ & $0.959_{(0.0041)}$ \\
			\bottomrule
		\end{tabular}
		\caption{Test accuracy on $P$. Subquantile is trained with $p = (1 - \eps)$ and max $32$ iterations. SEVER is trained with $8$ iterations and $p=0.1$. Noisy labels are flipped from $-1$ to $1$ or $-1$ to $1$.}
		\label{tab:svm-enron-salary}
	\end{table}
	
	
	Subquantile Minimization has near optimal performance across all noise regimes in the \texttt{Adult Dataset} from the UCI Machine Learning Repository \cite{Dua2019}.
	...
	Let $\vx_1,\vx_2,\cdots,\vx_n$ be ordered such that $f(\vx_1;\vtheta,y_1) \leq f(\vx_2;\vtheta,y_2) \leq \cdots \leq f(\vx_n;\vtheta,y_n)$. To simplify notation, we will denote $\hat{\vnu}$ as the vector of the losses such that $\hat{\vnu_1} = f(\vx_1;\vtheta,y_1)$.
	\begin{align}
		...
		&= 0 \text{ when }\displaystyle t = \hat{\vnu}_{np}&&
	\end{align}
	This is the $p$-quantile of the loss. Assuming no two points have equivalent losses, this means the minimizing value for $t$ has a range of values, $\hat{\vnu}_{np} \leq t < \hat{\vnu}_{np+1}$. This means $g(t,\vtheta)$ is not strongly convex with respect to $t$. 
	
	\subsection{Derivation of Lemma~\ref{lem:gthetaderiv}}
	\label{app:gthetaderiv}
		Let $f$ be a convex loss function. We choose $t_{(k+1)}$ as defined in lemma \ref{lem:gfermat} such that $\hat{\vnu}_{np} \leq t_{(k+1)} \leq \hat{\vnu}_{np+1}$, where $\hat{\vnu}_1 \leq \hat{\vnu}_2 \leq \cdots \leq \hat{\vnu}_n$ and $\vnu_i = f(\vx_i;\vtheta, y_i)$ for $ i = 1,\dots,n$. Now we can calculate the derivative:
		\begin{align*}
			\nabla_{\vtheta} g(t_{(k+1)},\vtheta_k) &= \nabla_\vtheta \left(t_{(k+1)} - \frac{1}{np}\sum_{i=1}^n\left(\vnu_{np} - f(\vx_i;\vtheta_{(k)},y_i)\right)^+\right) &&\\
			&= - \frac{1}{np}\sum_{i=1}^n\nabla_{\vtheta_k}\left(t_{(k+1)} - f(\vx_i;\vtheta_{(k)},y_i)\right)^+ &&\\				
			&= \frac{1}{np}\sum_{i=1}^{n}\nabla_\vtheta f(\vx_i;\vtheta_{(k)},y_i) \left\{
			\begin{array}{lr}
				1, & \text{if } t > \vnu_i\\
				0, & \text{if } t < \vnu_i \\
				\left[0,1\right], & \text{if } t = \vnu_i \\
			\end{array} \right\} &&
		\end{align*}
		Now we can note $\vnu_{np} \leq t_{(k+1)} \leq \vnu_{np+1}$.
		\begin{equation}
			\nabla_\vtheta g(t_{(k+1)},\vtheta_{(k)}) = \frac{1}{np}\sum_{i=1}^{np}\nabla_\vtheta f(\vx_i;\vtheta_{(k)},y_i)
		\end{equation}
	...
	\section{Theory for \textsc{Subq-Base} \ref{alg:sqo-ridge}}\label{app:Subquantile-optimization}
	...
	We will describe the inliers and outliers:\\
	Inliers: $x_{\operatorname*{in}} \sim \mathcal{N}(0,I_2)$, $\vw \sim \mathcal{N}(0,I_2)$, $b \sim \mathcal{N}(0,1)$, $y|\vx \sim \operatorname*{sign}\left(\mathcal{N}(\vx^\top\vw +b, 0.01)\right)$\\
	Outliers: $x_{\operatorname*{out}} \sim \mathcal{U}\left(\left[-10,10\right]^2\right)$, $y|\vx \sim \operatorname*{sign}\left(-\vx^\top\vw -b\right)$\\
	$x_{\operatorname*{train}} = 800$, $X_{\operatorname*{test}} = 200$\\
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			\toprule 
			\textbf{Objectives}&\multicolumn{4}{c}{Test Accuracy (\texttt{Support Vector Machine})}\\                   
			\cmidrule(rl){2-5}
			&\subhead{$\eps = 0.1$}& \subhead{$\eps = 0.2$}& \subhead{$\eps = 0.3$} & \subhead{$\eps = 0.4$}\\ 
			\midrule
			...
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.919_{(0.0909)}}$&$0.728_{(0.1434)}$&$\mathbf{0.825_{(0.1643)}}$ & $0.739_{(0.1750)}$ \\
			\midrule 
			Genie ERM &$1.000_{(0.000)}$&$1.000_{(0.000)}$ & $1.000_{(0.000)}$& $1.000_{(0.000)}$\\
			\bottomrule
		\end{tabular}
		\caption{\texttt{SVM Synthetic}. Test accuracy on $P$. Subquantile is trained with $p = (1 - \eps)$ and max $32$ iterations. SEVER is trained with $4$ iterations and $p=0.3$.}
		\label{tab:svm-synthetic}
	\end{table}
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			\toprule 
			\textbf{Objectives}&\multicolumn{4}{c}{Test Accuracy (\texttt{Enron Spam Classification})}\\                   
			\cmidrule(rl){2-5}
			&\subhead{$\eps = 0.1$}& \subhead{$\eps = 0.2$}& \subhead{$\eps = 0.3$} & \subhead{$\eps = 0.4$}\\ 
			\midrule
			ERM  &$0.751_{(0.0156)}$&$0.633_{(0.0246)}$ & $0.524_{(0.0291)}$ & $0.496_{(0.0608)}$\\
			TERM \cite{li2020tilted} &$\infty$&$\infty$ & $\infty$ & $\infty$\\
			SEVER \cite{DiakonikolasKKLSS19} &$0.795_{(0.0167)}$&$0.649_{(0.0276)}$ & $0.567_{(0.0185)}$ & $0.528_{(0.0305)}$\\
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.892_{(0.0164)}}$&$\mathbf{0.815_{(0.0204)}}$&$\mathbf{0.750_{(0.0460)}}$ & $\mathbf{0.653_{(0.0490)}}$ \\
			\midrule 
			Genie ERM &$0.963_{(0.0065)}$&$0.963_{(0.0049)}$&$0.960_{(0.0072)}$ & $0.963_{(0.0060)}$ \\
			\bottomrule
		\end{tabular}
		\caption{Test accuracy on $P$. Subquantile is trained with $p = (1 - \eps)$ and max $32$ iterations. SEVER is trained with $8$ iterations and $p=0.1$. Noisy labels are flipped from $-1$ to $1$ or $-1$ to $1$.}
		\label{tab:svm-enron}
	...
	\begin{table}[!h]
		...
		\begin{tabular}{lcccc}
			\toprule 
			\textbf{Objectives}&\multicolumn{4}{c}{Test Accuracy (\texttt{Adult Dataset})}\\                   
			\cmidrule(rl){2-5}
			&\subhead{$\eps = 0.1$}& \subhead{$\eps = 0.2$}& \subhead{$\eps = 0.3$} & \subhead{$\eps = 0.4$}\\ 
			\midrule
			ERM  &$0.838_{(0.0029)}$&$0.785_{(0.0040)}$ & $0.781_{(0.0040)}$ & $0.776_{(0.0033)}$\\
			TERM \cite{li2020tilted} &$\infty$&$\infty$ & $\infty$ & $\infty$\\
			SEVER \cite{DiakonikolasKKLSS19} &$0.691_{(0.0545)}$&$0.681_{(0.0586)}$ & $0.587_{(0.0917)}$ & $0.551_{(0.1020)}$\\
			\rowcolor{LightCyan}
			Subquantile (Ours) &$\mathbf{0.847_{(0.0028)}}$&$\mathbf{0.841_{(0.0185)}}$&$\mathbf{0.842_{(0.0039)}}$ & $\mathbf{0.811_{(0.0040)}}$ \\
			\midrule 
			Genie ERM &$0.847_{(0.0028)}$&$0.847_{(0.0033)}$ & $0.848_{(0.0032)}$ & $0.848_{(0.0033)}$\\
			\bottomrule
		\end{tabular}
		\caption{Test accuracy on $P$. Subquantile is trained with $p = (1 - \eps)$ and max $32$ iterations. SEVER is trained with $8$ iterations and $p=0.1$. Noisy labels are flipped from $-1$ to $1$ or $-1$ to $1$.}
		\label{tab:svm-salary}
	\end{table}
	...
	$\vw \sim \mathcal{N}(4,4)^{200}$\\
	$b \sim \mathcal{N}(4,4)$\\
	$\vw' \sim \mathcal{N}(4,4)^{200}$\\
	$b' \sim \mathcal{N}(4,4)$\\
	$n_{\text{train}} = 1\text{e}4$\\
	$\mathbb{P}: y|\vx \sim \mathcal{N}(\vx^\top\vw + b, 0.1)$\\
	$\mathbb{Q}: y|\vx \sim \mathcal{N}(\vx^{'\top}\vw + b', 0.1)$\\
	Please note $\vw$, $b$, $\vw'$, $b'$, are all sampled independently. The noise is added after normalization of the dataset to the standard normal $\mathcal{N}(0,1)$. 
	...
	$\vw \sim \mathcal{N}(4,4)^{500}$\\
	$b \sim \mathcal{N}(4,4)$\\
	$\vw' = \vzero$\\
	$b' \sim \mathcal{N}(5,5)$\\
	$n_{\text{train}} = 8\text{e}3$\\
	$n_{\text{test}} = 2\text{e}3$\\
	$\mathbb{P}: y|\vx \sim \mathcal{N}(\vx^\top \vw  + b, 0.01)$\\
	$\mathbb{Q}: y|\vx \sim \mathcal{N}(5,5)$\\
	Please note $\vw$, $b$, $\vw'$, $b'$, are all sampled independently. The noise is added after normalization of the dataset to the standard normal. 
