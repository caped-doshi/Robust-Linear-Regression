...
\begin{document}
	...
	\begin{definition}
		...
		\begin{equation}
			U = \mathbb{E}\left[\ell \left( f(\vx;\vtheta,y)\right)\right]
		\end{equation}
	\end{definition}
	...
	\begin{algorithm}[H]
		...
		$\vtheta_{(0)} \gets (X^\top X)^{-1}X^\top y$\\
		\For{$k \in 1,2,\ldots,T$}
		{
			...
			$S_{(k)} \gets \concat \vx_i\text{ if } (\vtheta_{(k)}^\top \vx_i - y_i)^2 \leq t_{(k+1)}$\\
			$L_{(k)} \gets \frac{1}{np}\norm{S^\top S}_2$\\
	...
	\begin{minipage}{0.48\textwidth}
		\begin{algorithm}[H]
			...
			$\vtheta_{(0)} \gets (X^\top X + \lambda I)^{-1}X^\top y$\\
			\For{$k \in \left\{1,2,\dots,T\right\}$}
			{
				$\vnu \gets (X \vtheta_{(k)} - y)^2$\\
				$t_{(k+1)} \gets \hat{\vnu}_{np}$\\
				$S_{(k)} \gets \concat \vx_i\text{ if } (\vtheta_{(k)}^\top \vx_i - y_i)^2 \leq t_{(k+1)} \text{ where $\concat$ represents the concatenation operator}$\\
				$\vtheta_{(k+1)} \gets (S^\top S + \lambda I)^{-1}S^\top y_S$\\
			}
			\KwRet{$ \vtheta_T $}
	...
	\begin{assumption}\label{asm:normal-corruption}
		...
		\begin{equation}\vspace{1em}
			\vy_P = \vbeta_P^\top P  + \eps_P
		\end{equation}
		and similarly 
		\begin{equation}
			\vy_Q = \vbeta_Q^\top Q + \eps_Q
		\end{equation}
	...
	We will use our assumptions to quantify the effect of the corrupted data on an optimal least squares regression model. We are interested in $\displaystyle (X^\top X)^{-1}X^\top y - (P^\top P)^{-1}P^\top y$.
	It is know the least squares optimal solution for $X$ is equal to $(X^\top X)^{-1}X^\top y$\\
	Note $\displaystyle X = \begin{pmatrix}P \\ Q \end{pmatrix}$ and $\displaystyle y = \begin{pmatrix} \vy_P \\ \vy_Q \end{pmatrix} $so $X^\top = \begin{pmatrix}P^\top & Q^\top \end{pmatrix}$\\
	...
	\begin{restatable}{thm}{expected-value-corrupted-different}\label{thm:expected-value-corrupted-different}
		...
		\begin{equation}
			\mathbb{E}\left[X^{\dagger}y \right] = \vbeta_P - n(1-\eps)\Sigma_P^{-1}\vbeta_Q
		\end{equation}
	\end{restatable}
	...
	\begin{align*}
		...
		2(\vmu\vmu^\top + \Sigma)(\vtheta_k - (1-\varepsilon)\vbeta_P-\varepsilon\vbeta_Q) &= 0&&
	\end{align*}
	...
	\begin{proofsketch}
		...
		\begin{equation}
			\argmin_{\vtheta \in \mathbb{R}^d}\min_{S \in \Pi}\norm{\vtheta^\top S - y_S}_2^2 \iff \argmin_{\vtheta \in \mathbb{R}^d}\max_{t \in \mathbb{R}}\norm{\vtheta^\top S - y_S}_2^2
		\end{equation}
	...
	\begin{lemma} For a hermitian matrix, it follows:
		\begin{equation*}
			\norm{\mA}_2 = |\lambda_1|
		\end{equation*}
	\end{lemma}
	...
	\end{equation*}
	Then we can make the following manipulations:
	\begin{align*}
		...
		= &\norm{\vbeta_P -  \left(P^\top P + Q^\top Q + \lambda I\right)^{-1}P^\top \vy_P  - \left(P^\top P + Q^\top Q + \lambda I\right)^{-1}Q^\top \vy_Q}_2 &&\\
		\leq &\norm{\vbeta_P}_2 + \norm{\left(P^\top P + Q^\top Q+ \lambda I\right)^{-1}P^\top \vy_P}_2 + \norm{\left(P^\top P + Q^\top Q+ \lambda I\right)^{-1}Q^\top \vy_Q}_2 &&\\
		\overset{\mathrm{Cauchy-Schwarz}}{\leq} &\norm{\vbeta_P}_2 + \norm{\left(P^\top P + Q^\top Q + \lambda I\right)^{-1}}_2\norm{P^\top \vy_P}_2 + \norm{\left(P^\top P + Q^\top Q + \lambda I\right)^{-1}}_2\norm{Q^\top \vy_Q}_2&&\\
		\overset{\mathrm{Cauchy-Schwarz}}{\leq} &\norm{\vbeta_P}_2 + \sqrt{\lambda_{\max}\left(P^\top P\right)}\norm{\left(P^\top P + Q^\top Q + \lambda I\right)^{-1}}_2\norm{\vy_P}_2 + \sqrt{\lambda_{\max}\left(Q^\top Q\right)}\norm{\left(P^\top P + Q^\top Q + \lambda I\right)^{-1}}_2\norm{\vy_Q}_2 &&\\
		= &\norm{\vbeta_P}_2 + \frac{\sqrt{\lambda_{\max}\left(P^\top P\right)}\norm{\vy_P}_2}{\lambda_{\max}\left(P^\top P + Q^\top Q + \lambda I\right)} + \frac{\sqrt{\lambda_{\max}\left(Q^\top Q\right)}\norm{\vy_Q}_2}{\lambda_{\max}\left(P^\top P + Q^\top Q + \lambda I\right)} &&\\
		= &\norm{\vbeta_P} + \frac{\sqrt{\lambda_{\max}\left(P^\top P\right)}\norm{\vy_P}_2 + \sqrt{\lambda_{\max}\left(Q^\top Q\right)}\norm{\vy_Q}_2}{\lambda_{\max}\left(P^\top P + Q^\top Q + \lambda I\right)} &&\\
		\leq & \norm{\vbeta_P}_2  + \frac{\sqrt{\lambda_{\max}\left(P^\top P\right)}\norm{\vy_P}_2 + \sqrt{\lambda_{\max}\left(Q^\top Q\right)}\norm{\vy_Q}_2}{\lambda_{\min}\left(P^\top P\right) + \lambda_{\min}\left(Q^\top Q\right) + \lambda} &&
	\end{align*}
	To simplify $\vy_P$ and $\vy_Q$ we will utilize 	
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \vtheta_k - \alpha\left(\sum_{\vp \in \mathcal{S}} \left(\vmu \vmu^\top + \Sigma\right)\vtheta_k - \left(\vmu \vmu^\top + \Sigma\right)\vbeta_P - \sum_{\vq \in \mathcal{S}} \left(\vmu \vmu^\top + \Sigma\right)\vtheta_k + \left(\vmu \vmu^\top + \Sigma\right)\vbeta_Q\right) &&
		\end{align*}
		 \vspace{1em}
		\begin{equation}\vspace{1em}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \left(\left(\vtheta^\top_{(t+1)}-\vbeta_P^\top\right)\vmu\right)^2 + \left(\vtheta^\top_{(t+1)} - \vbeta_P^\top\right)\Sigma\left(\vtheta_{(t+1)} - \vbeta_P\right) + \Var\left(\eps_P\right)
		\end{align*}
		It thus follows similarly:
		\begin{equation*}
			\mathbb{E}\left[\left(\vtheta_{(t+1)}^\top\vq_i - y_i\right)^2\right] =  \left(\left(\vtheta^\top_{(t+1)}-\vbeta_Q^\top\right)\vmu\right)^2 + \left(\vtheta^\top_{(t+1)} - \vbeta_Q^\top\right)\Sigma\left(\vtheta_{(t+1)} - \vbeta_Q\right) + \Var\left(\eps_Q\right)
		\end{equation*}
		...
		Let $\sigma^2_P \coloneqq \Var\left(\vtheta^\top_{(t+1)}\vp_i - y_i\right) = \left(\vtheta^\top_{(t+1)} - \vbeta_P^\top\right)\Sigma\left(\vtheta_{(t+1)} - \vbeta_P\right) + \Var\left(\eps_P\right)$. 
		\begin{align*}
	...
	In both the \texttt{Cal-Housing} and \texttt{Abalone} datasets there exists feature and label noise that exist with $5\%$ probability. In this the case, the probability is low, however since the noise is very large, even having a few points from $\mathbb{Q}$ in the final subquantile matrix can largely the bias the predictions away from the optimal parameters for $\mathbb{P}$. Therefore, we reduce $p$, the size of the subquantile to reduce the probability of obtaining corrupted samples within the subquantile. However, what we get in a decrease in variance, we do increase the bias error, albeit very slightly. 
		...
		The results in figure \ref{fig:structure-subquantile} demonstrate the percentage of elements from $\mathbb{Q}$ in the final subquantile matrix $S_T$ from different theta initializations after $T$ iterations of subquantile minimization in the \texttt{Noisy Linear Regression} Dataset. 
