...
\begin{document}
	...
	\subsection{Corruption on Ridge Regression}
	\begin{assumption}\label{asm:normal-sampling}
		The rows of $\mP$ and $\mQ$ are sampled from $\vzero$ centered Normal Distributions.
		\begin{equation*}
			\mP_i \sim \mathcal{N}\left(\vzero, \mSigma_P\right)
		\end{equation*}
		\begin{equation}
			\mQ_i \sim \mathcal{N}\left(\vzero, \mSigma_Q\right)
		\end{equation}
	\end{assumption}
	
	\begin{assumption}\label{asm:wishart}
		By assumption \ref{asm:normal-sampling}, it thus follows that the matrices $\mP^\top\mP$ and $\mQ^\top\mQ$ are sampled from Wishart Distributions.
		\begin{equation}
			\mP^\top\mP \sim \mathcal{W}\left(n,\mSigma_P\right)
		\end{equation}
		\begin{equation}
			\mQ^\top\mQ \sim \mathcal{W}\left(n, \mSigma_Q\right)
		\end{equation}
	\end{assumption}
	...
	\begin{equation}
		\hat{\vbeta} = \left(\mX^\top\mX+ \lambda \mI\right)^{-1}\mX^\top\vy
	\end{equation}
	We will solve for the expected solution given $\varepsilon^{(t)}$
	\begin{align*}
		\mathbb{E}&\left[\left(\mX^\top\mX+ \lambda \mI\right)^{-1}\mX^\top\vy\right] = \mathbb{E}\left[\left(\mP^\top \mP + \mQ^\top \mQ + \lambda \mI\right)^{-1}\mP^\top\vy_p + \left(\mP^\top \mP + \mQ^\top \mQ + \lambda \mI\right)^{-1}\mQ^\top\vy_q\right]&&\\
		&= \mathbb{E}\left[\left(\mP^\top \mP + \mQ^\top \mQ + \lambda \mI\right)^{-1}\mP^\top\left(\mP \vbeta_P + \eps\right) + \left(\mP^\top \mP + \mQ^\top \mQ + \lambda \mI\right)^{-1}\mQ^\top\left(\mQ \vbeta_Q + \eps\right)\right] &&\\
		&= \left(n\left(1-\varepsilon^{(t)}\right)\mSigma_P + n\varepsilon^{(t)}\mSigma_Q + \lambda \mI\right)^{-1}n\left(1-\varepsilon^{(t)}\right)\mSigma_P\vbeta_P + \left(n\left(1-\varepsilon^{(t)}\right)\mSigma_P + n\varepsilon^{(t)}\mSigma_Q + \lambda \mI\right)^{-1}n\varepsilon^{(t)}\mSigma_Q \vbeta_Q
	\end{align*}
	Now we will utilize assumption \ref{asm:identity-covariance}.
	\begin{align*}
		&= \left(n\left(1-\varepsilon^{(t)}\right)\xi_P\mI + n\varepsilon^{(t)}\xi_Q\mI + \lambda \mI\right)^{-1}n\left(1-\varepsilon^{(t)}\right)\xi_P\mI\vbeta_P + \left(n\left(1-\varepsilon^{(t)}\right)\xi_P\mI + n\varepsilon^{(t)}\xi_Q\mI + \lambda \mI\right)^{-1}n\varepsilon^{(t)}\xi_Q\mI \vbeta_Q &&\\
		&= \left(n\left(\left(1-\varepsilon^{(t)}\right)\xi_P + \varepsilon^{(t)}\xi_Q\right) + \lambda\right)^{-1}n\left(1-\varepsilon^{(t)}\right)\vbeta_P + \left(n\left(\left(1-\varepsilon^{(t)}\right)\xi_P + \varepsilon^{(t)}\xi_Q\right) + \lambda\right)^{-1}n\varepsilon^{(t)}\vbeta_Q &&\\
	\end{align*}
	We will now represent the fractional form
	\begin{align*}
		&= \frac{n\left(1 - \varepsilon^{(t)}\right)\xi_P\vbeta_P + n\varepsilon^{(t)}\xi_Q\vbeta_Q}{n\left(\left(1-\varepsilon^{(t)}\right)\xi_P + \varepsilon^{(t)}\xi_Q\right) + \lambda} &&\\
		&= \frac{\left(1 - \varepsilon^{(t)}\right)\xi_P\vbeta_P + \varepsilon^{(t)}\xi_Q\vbeta_Q}{\left(1-\varepsilon^{(t)}\right)\xi_P + \varepsilon^{(t)}\xi_Q + \frac{\lambda}{n}} &&\\
	\end{align*}
	In the simple case, where we have $0$ regularization and $\xi_P = \xi_Q = \mI$, the equation simplifies to:
	\begin{equation}
		\hat{\vbeta} = \left(1-\varepsilon^{(t)}\right)\vbeta_P + \varepsilon^{(t)}\vbeta_Q
	\end{equation}
	With this representation, we will be able to give strong theoretical guarantees for algorithm \ref{alg:sqo-ridge}.
